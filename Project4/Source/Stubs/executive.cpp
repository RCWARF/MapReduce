// Starts stub threads at the given endpoints
#include <string>
#include <sstream>
#include <iostream>
#include <thread>
...
StubExecutive::StubExecutive(std::string map_dll_file,
	...
	// Load DLLs
	getMapHandler(map_dll_file);
	getReduceHandler(reduce_dll_file);
...
int StubExecutive::runStubs(std::vector<std::string> stub_endpoints) {
	// Start stub threads
	createStubs(stub_endpoints);
	return 0;
}
...
void StubExecutive::getMapHandler(std::string path_to_dll)
{
	...
	else
	{
		...
		if (hDLL_map_ != NULL) {
			...
			create_map_ = (CREATMAP)GetProcAddress(hDLL_map_, "createMapper");
...
void StubExecutive::getReduceHandler(std::string path_to_dll)
{
	...
	else
	{
		...
		if (hDLL_reduce_ != NULL) {
			...
			create_reduce_ = (CREATREDUCE)GetProcAddress(hDLL_reduce_, "createReducer");
...
void StubExecutive::createStubs(std::vector<std::string> stub_endpoints) {
	...
	BOOST_LOG_TRIVIAL(info) << "Stub threads terminated.";
}

void StubExecutive::stubProc(std::string endpoint, const CREATMAP& map, const CREATREDUCE& reduce)
{
	Stub stub(endpoint, map, reduce);
	stub.runStub();
	BOOST_LOG_TRIVIAL(info) << "stubProc terminiated.";
}
