//	Use Comm socket and run map/reduce processes
...
// Creates a map request message to pass to an endpoint
MsgPassingCommunication::Message Workflow::createStopMessage(const MsgPassingCommunication::EndPoint& destination,
	const std::vector<boost::filesystem::path>& files,
	int partitions)
...
void Workflow::runMapReduceWf()
{
	...
	for (int m = 0; m < map_partitions.size(); m++) {
		...
		BOOST_LOG_TRIVIAL(info) << "Message contents: " << map_message.toString();
	...
	for (int r = 0; r < reduce_partitions.size(); r++) {
		...
		BOOST_LOG_TRIVIAL(info) << "Sent reduce request to " << this->stubs_[stub].toString();
		BOOST_LOG_TRIVIAL(info) << "Message contents: " << reduce_message.toString();
		stub = (stub + 1) % this->stubs_.size();
	}
	...
	this->controller_->postMessage(reduce_message);
	BOOST_LOG_TRIVIAL(info) << "Sent the final reduce operation request to " << this->stubs_[stub].toString();
	BOOST_LOG_TRIVIAL(info) << "Message contents: " << reduce_message.toString();
	...
	BOOST_LOG_TRIVIAL(info) << "Final Reduce operation process successfull: " << received_message.attributes()["message"];
	...
	// Notify server to stop the client 
	MsgPassingCommunication::Message stop_message;
	stub = 0;
	for (int m = 0; m < map_partitions.size(); m++) {
		stop_message = createStopMessage(this->stubs_[stub], map_partitions[0], reducer_count);
		this->controller_->postMessage(stop_message);
		BOOST_LOG_TRIVIAL(info) << "Sent stop request to " << this->stubs_[stub].toString();
		//BOOST_LOG_TRIVIAL(info) << "Message contents: " << stop_message.toString();
		stub = (stub + 1) % this->stubs_.size();
	}	
}
